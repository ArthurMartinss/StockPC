document.getElementById("submit_fis").onclick=async (e)=>{
    ...
    if(cpf.value.length!=14||!testaCPF(cpf.value)){
...
document.getElementById("submit_jur").onclick=async (e)=>{
    ...
    if(cnpj.value.length!=18||!testaCNPJ(cnpj.value)){
...
const cpfMask = (value) => {
    ...
    // Insert dots and hyphen at the appropriate positions
...
const cnpjMask = (value) => {
    ...
    // Insert dots, a slash, and a hyphen at the appropriate positions
    value = value.replace(/^(\d{2})(\d)/, "$1.$2");
...
const handlePhoneNumber = (event) => {
  ...
  const phoneNumberMask = (value) => {
    ...
    // Insert parentheses, a space, and a hyphen at the appropriate positions
    value = value.replace(/^(\d{2})(\d)/, "($1) $2");
    value = value.replace(/(\d{4})(\d)/, "$1-$2");
...
async function verificarEmailBd(email) {
    ...
    let url = "https://stockpc.store/controller/ajax_verificar_email_bd.php?email=" + email;
